...
# The rebuild hint both rendered headers carry. Install-mode-neutral on purpose: a plugin install has no
# project bin/ (the engines live in the plugin cache and the skills resolve them), so a bare
# `python3 bin/build_ticket_index.py` hint sent a fresh plugin user to "No such file or directory".
# The skill is the primary route; the script path is the vendored/pip alternative.
REGEN_HINT = [
    "     Rebuild after adding or closing a ticket: `/refresh index` (`/ticketwright:refresh index` on a plugin",
    "     install). pip installs: `ticketwright index`; a vendored repo (after `ticketwright init`) can run `python3 bin/build_ticket_index.py`. -->",
]
...
def main() -> int:
    ...
    if args.prune:
        ...
        print("Now re-render: /refresh index (plugin: /ticketwright:refresh index) · pip: ticketwright index · vendored: python3 bin/build_ticket_index.py")
        return 0
    ...
    if args.check:
        ...
        if stale:
            print(f"stale: {', '.join(stale)} — rebuild: /refresh index (plugin: /ticketwright:refresh index) · pip: ticketwright index · vendored: python3 bin/build_ticket_index.py", file=sys.stderr)
            return 1
