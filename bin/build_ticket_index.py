#!/usr/bin/env python3
"""Render tickets/INDEX.md — the master catalog of every ticket in this repo.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Render tickets/INDEX.md")
    ap.add_argument("--check", action="store_true", help="exit 1 if INDEX.md is stale vs a fresh render")
    ...
    if args.stats:
        ...
        print(f"discovered: {len(rows)}  enriched: {len(rows) - len(un)}  un-enriched: {len(un)}  stale: {len(st)}")
        if un:
    ...
    if args.check:
        ...
        if current != fresh:
            ...
            return 1
        print("tickets/INDEX.md is up to date.")
        return 0

    if not (root / "tickets").is_dir():
        print("No tickets/ directory yet — nothing to index.", file=sys.stderr)
        return 0
    index_path.write_bytes(fresh.encode("utf-8"))  # write_bytes => stable \n line endings everywhere
    un = sum(1 for r in rows if not r["enriched"])
    print(f"Wrote {index_path.relative_to(root)} — {len(rows)} tickets ({un} un-enriched).", file=sys.stderr)
    return 0
