...
|---|---|---|
| `tickets/index_data.json` | Enrichment store: per-ticket `title`, `status`, `date`, one-line `summary`, `tags`, `cross_refs`, plus each README's `readme_hash` at enrichment time. | model (build-ticket-index skill / close step) |
| `bin/build_ticket_index.py` | **Renderer.** Discovers every ticket folder, merges the store, writes `tickets/INDEX.md`. Deterministic, LLM-free, stdlib-only. | — |
...
- `ticket_url_template` — e.g. `https://acme.atlassian.net/browse/{id}`; how `INDEX.md` links each
  ticket. Omit/`null` for trackers without a stable URL (the index just drops the `↗` link).
...
Then commit `tickets/INDEX.md` + `tickets/index_data.json` with the ticket. An agent closing the
